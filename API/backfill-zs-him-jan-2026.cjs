#!/usr/bin/env node
/**
 * Backfill: ZS-HIM flights for January 2026, compute TMNP violations, write to disk cache.
 *
 ...
 *   PORT=4602 FR24_TRACK_MIN_INTERVAL_MS=6500 FR24_VIOLATIONS_LOOP_DELAY_MS=6500 node API/server.cjs
 *   node API/backfill-zs-him-jan-2026.cjs
 */

const BASE_URL = process.env.BASE_URL || 'http://localhost:4602';
const REG = 'ZS-HIM';
...
function* windowsJan2026() {
  ...
  // Safety: cap at "now" in case this is run mid-month.
  const now = new Date();
  const endExclusive = new Date(Math.min(monthEndExclusive.getTime(), now.getTime()));
