#!/usr/bin/env node
/**
 * Backfill: ZS-HIM flights for January 2025, compute TMNP violations, write to disk cache.
 *
 ...
 *   PORT=4601 FR24_TRACK_MIN_INTERVAL_MS=6500 FR24_VIOLATIONS_LOOP_DELAY_MS=6500 node API/server.cjs
 *   node API/backfill-zs-him-jan-2025.cjs
 */

const BASE_URL = process.env.BASE_URL || 'http://localhost:4601';
// User asked for "ZSHIM" (no dash). We use the canonical registration with dash.
const REG = 'ZS-HIM';
...
function* windowsJan2025() {
  // FR24 Flight Summary max is 14 days per request.
  const start = new Date(Date.UTC(2025, 0, 1, 0, 0, 0));
  const monthEndExclusive = new Date(Date.UTC(2025, 1, 1, 0, 0, 0));

  // Safety: don’t query into the future if someone reuses this script later.
  const now = new Date();
...
async function main() {
  ...
  // Quick sanity check that server is up and FR24 auth works.
  ...
  for (const w of windowsJan2025()) {
    ...
    // Small pause between summary calls (cheap, but polite).
    await sleep(250);
  }

  const ids = Array.from(allIds);
  console.log(`[backfill] Total unique Jan 2025 flights: ${ids.length}`);
