...
function loadTMNPCoordinates() {
  const tmnpKmlPath = path.join(__dirname, '..', '..', 'static-site', 'tmnp.kml');
...
if (positionalArgs.length > 0) {
  ...
  generateFlightImage(filename)
    .then(success => {
      if (success) {
        console.log(`✅ Successfully generated PNG for ${filename}`);
        process.exit(0);
      } else {
        console.log(`❌ Failed to generate PNG for ${filename}`);
        process.exit(1);
      }
    })
    .catch(error => {
