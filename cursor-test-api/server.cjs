#!/usr/bin/env node
/**
 * Tiny local server for cursor-test-api pages.
 *
 ...
 *
 * Usage:
 *   node cursor-test-api/server.cjs
...
function readOpenSkyCredentialsFromFile() {
  ...
  const clientId = String(json?.clientId || json?.client_id || '').trim();
  const clientSecret = String(json?.clientSecret || json?.client_secret || '').trim();
...
function readFR24TokenFromFile() {
  ...
  // - JSON containing token fields
  try {
    const json = JSON.parse(raw);
    const token = String(json?.token || json?.fr24Token || json?.apiToken || '').trim();
    return token || null;
  } catch {
    return raw;
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/opensky/token') {
      ...
      if (!clientId || !clientSecret) {
        return sendJson(res, 400, {
          ok: false,
          error: 'Missing OpenSky credentials (provide in request or create cursor-test-api/credentials.json)'
        });
      }
    ...
    if (req.method === 'GET' && url.pathname === '/api/fr24/status') {
      ...
      if (!token) return sendJson(res, 400, { ok: false, error: 'Missing FR24 token in cursor-test-api/credentials.json' });
...
server.listen(PORT, () => {
  console.log(`cursor-test-api server running at http://localhost:${PORT}/opensky.html`);
});
